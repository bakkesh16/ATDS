...
const InsuranceChatPage = () => {
  ...
  const handleSendMessage = () => {
    ...
    const url = 'http://localhost:8083/sendMessageToHospital';
  ...
  const fetchIncomingMessages = () => {
    axios
      .get('http://localhost:8083/receiveMessageFromHospital')
      .then((response) => {
        if (typeof response.data === 'string' && response.data.trim() !== '') {
          ...
          setMessages((prevMessages) => [
            ...prevMessages,
            { text: incomingMessage, sender: 'Hospital Admin' },
          ]);
        }
      })
      .catch((error) => {
  ...
  return (
    <React.Fragment>
      <Navbar2  homepage = 'InsuranceAdminHome'></Navbar2>
      <div
    style={{
...
export default InsuranceChatPage;
