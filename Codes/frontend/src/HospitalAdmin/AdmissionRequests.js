...
export default function AdmissionRequests() {
    ...
    useEffect(() => {
        const fetchData = async () => {
          try {
            const response = await axios.get('http://localhost:9094/findallrequestsbystatus/REQUESTED');
    ...
    const handleApprove = (e) => {
        if (Object.keys(rowSelection).length > 0) {
            ...
            axios.post('http://localhost:9094/approvepatientrequests', selectedKeys)
  .then(response => {
  ...
  .catch(error => {
    ...
    const handleReject = (e) => {
        if (Object.keys(rowSelection).length > 0) {
            ...
            axios.post('http://localhost:9094/rejectpatientrequests', selectedKeys)
            .then(response => {
              console.log(response.data);
            })
            .catch(error => {
